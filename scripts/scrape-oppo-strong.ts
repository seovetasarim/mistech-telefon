...
const START_URL = "https://euromobilecompany.de/de/parts/oppo";
...
function normalizeCategoryUrl(u: string): string {
  try {
    ...
    if (!/\/de\/parts\/oppo(\/|$)/i.test(path)) return "";
    url.pathname = path;
    return url.toString();
  } catch {
    return "";
  ...
  while (pageUrl && pageCount < MAX_PAGES) {
    ...
    if (items.length === 0) {
      ...
      for (const it of items) {
        const isValidUrl = it.url && /\/de\/parts\/oppo\//i.test(it.url);
...
async function main() {
  ...
  const outPath = "public/data/oppo-import.json";
