...
async function main() {
  const root = "https://euromobilecompany.de/de/accessories/cases-and-covers";
  ...
  const brandFilters = ["For Apple","Samsung","Xiaomi","Huawei","Google","OnePlus","Motorola","Nokia","Realme","Vivo","Sony","LG","Microsoft","Lenovo","ASUS","Wiko","ZTE","Nintendo"];
  ...
  async function crawlCategory(url: string) {
    ...
    const label = b || decodeURIComponent(new URL(url).pathname.split("/").filter(Boolean).pop() || "cases-and-covers");
    const all: Item[] = [];
  ...
  await writeFile("public/data/accessories-cases-and-covers.json", JSON.stringify(categories, null, 2), "utf8");
