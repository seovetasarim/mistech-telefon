      ...
      try {
        ...
        for (const n of nodes) {
          ...
          if (t.includes("product")) {
            ...
            if (p) { const cleaned = String(p).replace(/[^0-9,.]/g, "").replace(/\./g, "").replace(",", "."); const num = Number(cleaned); if (Number.isFinite(num)) price = Math.round(num * 100); }
...
async function main() {
  const root = "https://euromobilecompany.de/de/accessories/holders";
  const html = await fetchHtml(root);
  const $ = cheerio.load(html);
  const catLinks = new Set<string>();
  $(".category-list a, .list-group a, .breadcrumb a, .content a").each((_, el)=>{ const href = $(el).attr("href") || ""; if (!href) return; const abs = new URL(href, root).toString(); if (abs.startsWith(root) && abs !== root) catLinks.add(abs); });
  const brandFilters = ["Rixus","White label","For Apple","Samsung","Xiaomi","Huawei","Google","OnePlus","Motorola","Nokia","Realme","Vivo","Sony","LG","Microsoft","Lenovo","ASUS","Wiko","ZTE","Nintendo"];
  const fmFilters = ["44","1"]; // user-specified filters
  // base root, brand-only, fm-only and brand+fm combinations
  ...
  async function crawlCategory(url: string) {
    ...
    const label = b || decodeURIComponent(new URL(url).pathname.split("/").filter(Boolean).pop() || "holders");
    const all: Item[] = [];
  ...
  await writeFile("public/data/accessories-holders.json", JSON.stringify(categories, null, 2), "utf8");
