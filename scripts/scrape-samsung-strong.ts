...
const START_URL = "https://euromobilecompany.de/de/parts/samsung";
...
function normalizeCategoryUrl(u: string): string {
  try {
    ...
    if (!/\/de\/parts\/samsung(\/|$)/i.test(path)) return "";
    url.pathname = path;
    return url.toString();
  } catch {
    return "";
  ...
  while (pageUrl && pageCount < MAX_PAGES) {
    ...
    if (items.length === 0) {
      ...
      for (const it of items) {
        const isValidUrl = it.url && /\/de\/parts\/samsung\//i.test(it.url);
...
function extractSubcategoryLinks($: cheerio.CheerioAPI): string[] {
  ...
  $("a[href]").each((_, a) => {
    ...
    if (!href) return;
    if (/\/de\/parts\/samsung\//i.test(href)) {
...
async function main() {
  ...
  const outPath = "public/data/samsung-import.json";
