...
function normalizeCategoryUrl(u: string): string {
  try {
    ...
    // Only keep path under for-iphone; strip queries and fragments
    url.search = "";
    url.hash = "";
    let path = url.pathname.replace(/\/+$/, "");
    if (!/\/de\/parts\/.*for-apple\/for-iphone/i.test(path)) return "";
    url.pathname = path;
    return url.toString();
  } catch {
    return "";
  ...
  while (pageUrl && pageCount < MAX_PAGES) {
    ...
    if (items.length === 0) {
      ...
      for (const it of items) {
        // Only accept product URLs within for-iphone to avoid site-level JSON-LD noise
        const isValidUrl = it.url && /\/de\/parts\/.*for-apple\/for-iphone\//i.test(it.url);
...
async function main() {
  ...
  while (queue.length) {
    ...
    try {
      ...
      // Discover more category links recursively
