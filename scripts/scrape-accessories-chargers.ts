...
async function scrapeDetail(url: string): Promise<Partial<Item>> {
  try {
    ...
    $("script[type='application/ld+json']").each((_, el) => { try { const data = JSON.parse($(el).contents().text() || "null"); const nodes = Array.isArray(data) ? data : [data]; for (const n of nodes) { const t = (n && (n["@type"] || "")).toString().toLowerCase(); if (t.includes("product")) { if (!title) title = n.name || title; const p = n.offers?.price ?? n.price; if (p) { const cleaned = String(p).replace(/[^0-9,.]/g, "").replace(/\./g, "").replace(",", "."); const num = Number(cleaned); if (Number.isFinite(num)) price = Math.round(num * 100); } if (!image) image = n.image || image; } } } catch {} });
...
async function scrapeList(url: string): Promise<{ items: Item[]; next?: string; pages?: string[] }> {
  ...
  $("a.product-thumb, .product-thumb a, .product-grid .product-thumb a, .image a, h4 a, .caption h4 a").each((_, el) => { const a = $(el); const href = a.attr("href"); const title = a.text() || a.attr("title") || ""; const img = a.find("img").attr("data-src") || a.find("img").attr("src") || a.closest(".product-thumb, .product-layout").find("img").attr("src") || ""; pushItem(href, title, img); });
...
async function main() {
  const root = "https://euromobilecompany.de/de/accessories/chargers";
  ...
  const brandFilters = ["For Apple","Samsung","Xiaomi","Huawei","Google","OnePlus","Motorola","Nokia","Realme","Vivo","Sony","LG","Microsoft","Lenovo","ASUS","Wiko","ZTE","Nintendo","Rixus","White label","App","Durata","Orten","Philips","Sunshine"];
  const fmFilters = ["44","8","18","58","13","54","25","1"];
  ...
  // explicit fm label mapping from user
  const fmLabel: Record<string, string> = { '44': 'Rixus', '8': 'App', '18': 'Durata', '58': 'Philips', '13': 'Samsung', '54': 'Sunshine', '25': 'Xiaomi', '1': 'White label' };
  for (const fm of Object.keys(fmLabel)) { const u = `${root}?fm=${fm}`; labelMap.set(u, fmLabel[fm]); }

  const categories: Category[] = [];
  async function crawlCategory(url: string) {
    const b = brandFromUrl(url);
    const fromMap = labelMap.get(url);
    const label = (fromMap && fromMap.trim()) || b || decodeURIComponent(new URL(url).pathname.split("/").filter(Boolean).pop() || "chargers");
    const all: Item[] = [];
  ...
  await writeFile("public/data/accessories-chargers.json", JSON.stringify(categories, null, 2), "utf8");
