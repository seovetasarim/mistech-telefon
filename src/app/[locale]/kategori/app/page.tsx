...
function buildCategoryGroups(cats: ImportedCategory[]) {
  ...
  for (const c of cats) {
    ...
    if (!label || label.toLowerCase() === "for iphone" || label.toLowerCase() === "for apple" || label.toLowerCase() === "parts") continue;
...
export default async function AppCategoryPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams?: Promise<{ model?: string; page?: string }> }) {
  ...
  for (const c of categoriesAll) {
    for (const it of c.items) {
      if (it?.id) allUniqueIds.add(it.id);
    }
  }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-8 md:py-10">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold">{modelQuery ? `App Kategorisi – ${modelQuery}` : "App Kategorisi"}</h1>
      ...
      <div className="mb-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        <aside className="lg:col-span-1">
          <div className="rounded-md border p-3">
            ...
            <div className="space-y-1 max-h-[70vh] overflow-auto pr-1">
              <a href={`/${locale}/kategori/app`} className={`block text-sm px-2 py-1 rounded ${!nq ? 'bg-accent/50' : 'hover:bg-accent/30'}`}>Tümü ({allUniqueTotal.toLocaleString('tr-TR')})</a>
              {groups.map((g, i) => {
                const active = normalize(g.label) === nq;
                const href = `/${locale}/kategori/app?model=${encodeURIComponent(g.label)}`;
                return (
        ...
        <div className="lg:col-span-3">
          ...
          {totalPages > 1 && (
            <Pagination current={pageNum} total={totalPages} makeHref={(p)=>`/${locale}/kategori/app?${new URLSearchParams({ ...(nq ? { model: modelQuery } : {}), page: String(p) }).toString()}`} />
          )}
        </div>
      </div>
