...
function labelFromUrl(u:string){ try{ const url=new URL(u); const parts=url.pathname.split("/").filter(Boolean); const last=parts.pop()||""; const label=decodeURIComponent(last).replace(/[-_]/g," ").trim(); if(!label||label.toLowerCase()==="google"||label.toLowerCase()==="parts"){ const prev=parts.pop()||label; return decodeURIComponent(prev).replace(/[-_]/g," ").trim(); } return label; } catch{ return ""; } }
...
export default async function GoogleCategoryPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams?: Promise<{ model?: string; page?: string }> }){
  const { locale } = await params;
  const s = (await (searchParams ?? Promise.resolve({} as any))) as { model?: string; page?: string };
  const catsAll=(await loadGoogle()).filter(c=>(c.items?.length??0)>0);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 md:px-6 py-8 md:py-10">
      <div className="mb-6"><h1 className="text-2xl font-semibold">{model?`Google Kategorisi – ${model}`:"Google Kategorisi"}</h1><p className="text-sm text-muted-foreground">Alt kategoriler ve ithal ürünler</p></div>
      <div className="mb-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
        <aside className="lg:col-span-1"><div className="rounded-md border p-3"><div className="font-medium mb-2">Alt Kategoriler</div><div className="space-y-1 max-h-[70vh] overflow-auto pr-1">
          <a href={`/${locale}/kategori/google`} className={`block text-sm px-2 py-1 rounded ${!nq?'bg-accent/50':'hover:bg-accent/30'}`}>Tümü ({allTotal.toLocaleString('tr-TR')})</a>
          {groups.map((g,i)=>{ const active=normalize(g.label)===nq; const href=`/${locale}/kategori/google?model=${encodeURIComponent(g.label)}`; return (<a key={g.label+i} href={href} className={`block text-sm px-2 py-1 rounded ${active?'bg-accent/50':'hover:bg-accent/30'}`}>{g.label} ({g.count.toLocaleString('tr-TR')})</a>); })}
        </div></div></aside>
        <div className="lg:col-span-3">
          ...
          {totalPages>1 && (<Pagination current={pageNum} total={totalPages} makeHref={(p)=>`/${locale}/kategori/google?${new URLSearchParams({ ...(nq?{model:model}:{}), page:String(p)}).toString()}`} />)}
        </div>
      </div>
    </div>
  );
