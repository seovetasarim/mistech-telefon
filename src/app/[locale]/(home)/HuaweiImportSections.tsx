...
async function loadHuawei(): Promise<ImportedCategory[]> {
  try { const fp = path.join(process.cwd(), "public", "data", "huawei-import.json"); const raw = await readFile(fp, "utf8"); const data = JSON.parse(raw); if (Array.isArray(data)) return data as ImportedCategory[]; } catch {}
  return [];
}

export default async function HuaweiImportSections({ locale }: { locale: string }) {
  const cats = await loadHuawei();
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-8">
      <div className="flex items-center justify-between mb-4"><h2 className="text-xl font-semibold">Huawei Ürünleri</h2></div>
      <BrandProductsSlider
        locale={locale}
