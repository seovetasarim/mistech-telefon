...
function labelFromUrl(u: string): string {
  try {
    ...
    return label;
  } catch { return "Kategori"; }
}

function groups(cats: ImportedCategory[]) {
  const m = new Map<string, { label: string; ids: Set<string> }>();
  for (const c of cats) {
    if (!c.items || c.items.length===0) continue; const lab = labelFromUrl(c.url); const k=lab.toLowerCase();
    if (!m.has(k)) m.set(k, { label: lab, ids: new Set<string>() });
    const g=m.get(k)!; for (const it of c.items) if (it?.id) g.ids.add(it.id);
  }
  return Array.from(m.values()).map(g=>({ label:g.label,count:g.ids.size})).sort((a,b)=>b.count-a.count);
}

export default async function OppoImportSections({ locale }: { locale: string }) {
  const cats = await loadOppo();
    ...
    <div>
      ...
      {products.length>0 && (
        <section className="mx-auto max-w-7xl px-6 py-8">
          <div className="flex items-center justify-between mb-4"><h2 className="text-xl font-semibold">Oppo Ürünleri</h2></div>
          <BrandProductsSlider
            locale={locale}
